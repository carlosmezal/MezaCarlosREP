...
RESPONSABILIDAD:
  Realizar Análisis Exploratorio de Datos (EDA) sobre cualquier
  pandas DataFrame: estadísticas, valores nulos, tipos de datos.

¿QUÉ ES EDA?
  El Análisis Exploratorio de Datos es el primer paso en todo
  proyecto de Ciencia de Datos. Nos permite:
  - Entender la estructura del dataset (filas, columnas, tipos)
  - Detectar valores faltantes (NaN) o inconsistentes
  - Ver la distribución estadística de los datos
  - Identificar problemas de calidad antes de modelar

CONCEPTOS CLAVE DE PANDAS:
  df.shape         → (filas, columnas) — dimensiones del DataFrame
  df.dtypes        → tipos de dato por columna (int64, float64, object)
  df.isnull()      → máscara booleana: True donde hay NaN
  df.describe()    → estadísticas descriptivas de columnas numéricas
  df.head(n)       → primeras n filas
  df.columns       → Index con nombres de columnas
"""
...
class DataAnalyzer:
    """
    Clase para analizar pandas DataFrames con enfoque en EDA.

    Principio OOP aplicado: Encapsulamiento.
    El DataFrame está "encapsulado" dentro de la clase.
    Solo se puede acceder a sus estadísticas a través de los
    métodos públicos definidos.

    Atributos
    ---------
    df : pd.DataFrame
        El DataFrame a analizar (solo lectura una vez inicializado).
    """
    ...
    def get_shape(self) -> dict:
        ...
        df.shape retorna una tupla: (número_de_filas, número_de_columnas)
        Ejemplo: (1200, 8) → 1200 filas, 8 columnas

        Retorna
        -------
        dict con claves 'registros' y 'columnas'
        """
    ...
    def get_columns(self) -> list:
        ...
        df.columns devuelve un pandas Index (similar a lista).
        list() lo convierte en lista Python normal.
    ...
    def get_dtypes(self) -> dict:
        ...
        Tipos más comunes en pandas:
          int64    → números enteros (1, 2, 100, -5)
          float64  → números decimales (9.2, 3.14, -0.5)
          object   → texto/strings ("Ana", "CDMX") o tipos mixtos
          bool     → booleanos (True, False)
          datetime64[ns] → fechas y horas

        df.dtypes devuelve una Series: índice=columna, valor=dtype
        .items() itera sobre pares (columna, dtype)
        str(dtype) convierte el dtype a string legible

        Retorna
        -------
        dict : {nombre_columna: tipo_dato_str}
        """
